...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User getByPhone(String phone) {
        ...
        Set<ClientPermission> permissions = userClientRelations.stream().map(u -> userRoleRelationMapper.listPermByClientId(u.getUserId(), u.getClientId()))
    ...
    private ClientPermission toPerms(List<ClientPermissionPO> list) {
        if (CollectionUtil.isEmpty(list)) {
            return null;
        }
        Set<String> collect = list.stream().map(ClientPermissionPO::getCode).collect(Collectors.toSet());
        return new ClientPermission(list.get(0).getClientId(), collect);
    }
}
